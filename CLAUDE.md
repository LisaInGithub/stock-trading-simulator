...
1. `scripts/fetch-market-data.mjs` pulls free daily OHLCV from Yahoo Finance's public chart endpoint (no key) for every ticker in `scripts/watchlist.json`, and derives SMA20/50, RSI14, 20-day range, and % change.
...
3. `scripts/build-prompt.mjs` combines the portfolio state + market data into a user prompt appended to the system prompt.
